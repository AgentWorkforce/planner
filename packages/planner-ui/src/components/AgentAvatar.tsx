 ...
 *
 * Visual indicator for a planning agent showing their current state.
 * Displays role-based emoji icon with state indicators (working dot, needs input ring, etc).
 *
 * Props:
 * - role: Agent role type (architect, coder, etc.)
 * - state: Current agent state (normal, working, needs_input, idle, error)
 * - size: Avatar size (sm: 20px, md: 32px, lg: 40px)
 * - onClick: Optional click handler
 * - currentActivity: Optional activity text for popover (e.g., "Processing user request")
 * - currentStep: Optional current step text for popover (e.g., "Analyzing dependencies")
 * - currentThought: Optional thought preview for popover (truncated to 100 chars)
 * - displayName: Optional agent display name (overrides role label)
 * - showTooltip: Whether to show tooltip on hover (default: true)
 *
 * When state is 'working' and activity/step/thought are available, displays a rich popover
 * with structured information. Otherwise shows a simple tooltip with agent name and state.
 */
...
const STATE_CONTAINER_STYLES: Record<AgentState, string> = {
  normal: 'opacity-100',
  working: 'opacity-100',
  needs_input: 'opacity-100 ring-2 ring-error/50 ring-offset-1 ring-offset-bg-secondary',
  idle: 'opacity-50',
  error: 'opacity-100',
};
...
function StateIndicator({
  ...
  switch (state) {
    case 'working':
      return (
        <div
          className={`absolute ${config.indicatorPosition} ${config.indicator} rounded-full bg-accent-green animate-pulse`}
        />
      );
    case 'needs_input':
      return (
        <div
          className={`absolute ${config.indicatorPosition} ${config.indicator} rounded-full border-2 border-error bg-transparent ${
    ...
    case 'error':
      return (
        <div
          className={`absolute ${config.indicatorPosition} ${config.indicator} flex items-center justify-center text-warning text-[10px]`}
        >
          ⚠
        </div>
      );
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <Tooltip>
        ...
        <TooltipContent
          side="top"
          className="bg-bg-tertiary text-text-primary border-border-subtle"
        >
          {tooltipContent}
        </TooltipContent>
      </Tooltip>
