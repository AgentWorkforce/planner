 ...
 * Standalone version - applies changes directly without agent revision.
 */
export function createChangeRequestHandlers(storage: PlanStorage) {
  return {
     ...
     * In standalone mode, applies suggested changes directly.
     */
    create: async (
      ...
      try {
        ...
        // Standalone mode: Apply changes directly (no agent revision)
     ...
     * In standalone mode, this is a no-op (no agent revisions).
     */
    acceptRevision: async (
      ...
      try {
        ...
        // In standalone mode, there are no agent revisions
